...
//        _thz_station_vals(nullptr)
    {
        ...
        _min_allowed_eig = _input("min_freq", "minimum freq squared", 100.);
...
//        // pressure load created and applied on the panel only
        ...
        MAST::ConstantFieldFunction *_p_cav_ax_f;
          p_cav_ax = new MAST::Parameter("p_cav_ax", 500.e3);
          _p_cav_ax_f = new MAST::ConstantFieldFunction("pressure", *p_cav_ax);

          MAST::BoundaryConditionBase *_p_load_ax;
        _p_load_ax = new MAST::BoundaryConditionBase(MAST::SURFACE_PRESSURE);
        _p_load_ax->add(*_p_cav_ax_f);
        _discipline->add_side_load(1, *_p_load_ax);
    }
    ...
    virtual void evaluate(const std::vector<Real> &dvars, // design variable
        ...
        // negative temperatures or reaches tha maximum number of steps or a negative eig val was found
        _if_neg_eig = false;
        ...
        if (_if_analysis) {
            libMesh::out << "weight is:  " << wt << std::endl;
        ...
        solve() {
            ...
            if (!if_continuation_solver) {
                ...
                *nd = nullptr;
                ...
                if (abs((*_obj._p_cav)()) > 1.e5) {
                    Real p0_ = (*_obj._p_cav)();
                    int n_steps =  1000;
                    for (unsigned int i = 0; i < n_steps; i++) {

                        (*_obj._p_cav)() = p0_ * (i + 1.) / (1. * n_steps);
                        libMesh::out
                                << "Load step: " << i
                                << "  : T = " << (*_obj._temp)()
                                << "  : p = " << (*_obj._p_cav)()
                        ...
                        if (_obj._sys->nonlinear_solver->converged == false){
                            libMesh::out << "solver diverged " << std::endl;
                            libmesh_error();
                        }
                    }
                }
                ...
                if (n_temp_steps) {
                    ...
                    solver.initialize((*_obj.p_cav_ax)());
...
//                         visualization
                        ...
                        if (!_obj._if_analysis) {
                            ...
                            if ((*min_eig < _obj._min_allowed_eig) && ((*_obj.p_cav_ax)() < max_temp)) {
                                _obj._if_neg_eig = true;
                                libMesh::out << " negative eigenvalue found" << std::endl;
                                (*_obj.p_cav_ax)() = max_temp;
                                break;
                            }
                        }

                        if (  (*_obj.p_cav_ax)() < 0.0 )   {
                            _obj._if_neg_eig = true;
