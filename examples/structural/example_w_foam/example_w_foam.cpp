...
//    libMesh::SerialMesh*                        _mesh;// create the mesh
    ...
    libMesh::ReplicatedMesh* _ref_mesh;
...
//        _mesh = new libMesh::SerialMesh(this->comm());
            ...
            _ref_mesh= new libMesh::ReplicatedMesh(this->comm()),
            _mesh= new libMesh::ReplicatedMesh(this->comm());

        // length of domain
        _length = _input("length", " ", 1);
        _width = _input("width", " ", 1);
...
//        constrained_vars[3] = 5;  // tz
    ...
    virtual void init_dvar(std::vector<Real>& x,
        ...
        if (nm.length()) {
            ...
            Real dvars_change  = 0.05;
            for (int i = 0; i < x.size();i++)
                x[i]  = x[i] + x[i]*dvars_change;
